...
- All 17 HTML pages include a persistent English/Hindi selector.
- All 17 pages include browser voice read-aloud and speech-input controls when supported by the device/browser.
- A Pages drawer provides direct navigation across all 17 pages, including Farmer Login (02A).
- The accessibility toolbar is client-side and does not require a separate service.
- This package contains no `.env` secrets or staff passwords.

## Running the frontend
Open `page01-landing.html` through a local HTTP server such as VS Code Live Server. If the backend is configured, the pages can use its `/api` endpoints.
