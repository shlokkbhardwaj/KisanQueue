...
export function positionInQueue(snapshot, tokenNumber, avgServiceMinutes) {
  ...
  if (!ACTIVE_TOKEN_STATUSES.has(mine.status)) {
    return { tokenStatus: mine.status, tokensAhead: null, aheadTokens: [], estimatedWaitMinutes: null };
  }
  ...
  if (mine.status === 'SERVING' || mine.status === 'CALLED') estimatedWaitMinutes = 0;
  else if (avgServiceMinutes) estimatedWaitMinutes = ahead.length * avgServiceMinutes;
