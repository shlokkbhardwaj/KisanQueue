...
(function () {
  ...
  var DICT = {
    ...
    'Farmer portal':'किसान पोर्टल','Operations & staff':'संचालन और स्टाफ','Landing / Home':'होम','Farmer registration':'किसान पंजीकरण','Slot booking':'स्लॉट बुकिंग','Queue console':'कतार कंसोल','Operations dashboard':'संचालन डैशबोर्ड','Reports & analytics':'रिपोर्ट और विश्लेषण','Settings':'सेटिंग्स','Staff login':'स्टाफ लॉगिन',
    'Centre Operator':'केंद्र ऑपरेटर','Supervisor':'सुपरवाइज़र','Admin':'एडमिन','Operator':'ऑपरेटर','Sign in':'साइन इन','Sign in as Operator':'ऑपरेटर के रूप में साइन इन करें','Sign in as Supervisor':'सुपरवाइज़र के रूप में साइन इन करें','Sign in as Admin':'एडमिन के रूप में साइन इन करें','Signing in…':'साइन इन हो रहा है…','Select a role first':'पहले एक भूमिका चुनें','The selected role does not match this staff account.':'चयनित भूमिका इस स्टाफ खाते से मेल नहीं खाती।','Invalid staff ID or password.':'अमान्य स्टाफ आईडी या पासवर्ड।','Welcome,':'स्वागत है,',
  ...
  function capture(root, currentLang) {
    ...
    while ((t = walker.nextNode())) {
      ...
      if (!originalText.has(t)) {
        var raw = t.nodeValue;
        originalText.set(t, currentLang === 'hi' ? direct(raw, 'en') : raw);
      }
      knownNodes.add(t);
    }
  ...
  function getCurrentFile() {
    return (location.pathname.split(/[\\/]/).pop() || 'page01-landing.html').toLowerCase();
  }

  function preferredHost() {
    return document.querySelector('.topbar-right, .header-right, .topbar, header') || document.body;
  }

  function bindSelect(select) {
    if (!select || select.dataset.kqI18nBound) return;
    select.dataset.kqI18nBound = '1';
  ...
  function attachControl() {
    ...
    // Staff login is intentionally kept clean; language is available throughout
    // the authenticated staff portal (including Settings) instead.
    if (getCurrentFile() === 'page11-staff-login.html') return;
    ...
    preferredHost().appendChild(box);
    bindSelect(box.querySelector('select'));
  }

  function init() {
    // Always capture the page's canonical English DOM first. This keeps
    // English available as the source of truth when navigating Hindi -> English.
    ...
    var mo = new MutationObserver(function (mutations) {
      ...
      mutations.forEach(function (m) {
        m.addedNodes.forEach(function (n) {
          if (n.nodeType === 1 && !n.classList.contains('kq-i18n-control')) {
            capture(n, lang);
            added.push(n);
          }
        });
  ...
  window.KQI18n = {
    getLanguage: function () { return localStorage.getItem(KEY) || 'en'; },
    setLanguage: function (lang) { apply(lang); },
    refresh: function (root) { translateRoot(root || document.body, localStorage.getItem(KEY) || 'en'); }
  };
